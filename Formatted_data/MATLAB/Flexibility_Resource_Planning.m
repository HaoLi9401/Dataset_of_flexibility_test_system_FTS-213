...
load('data_time_series_20200214.mat');
load('month_week');
load('DCtrans');
load('TieLine2');
...
a.gen(:,30) = a.gen(:,30)/1e6; % M$ 火电发电相关成本
...
z_RCR = (sum(sum(sum(PRE_day.*M_days_RE)))-sum(sum(sum(z_PG_RE.*M_days_RE))))/sum(sum(sum(PRE_day.*M_days_RE)));
